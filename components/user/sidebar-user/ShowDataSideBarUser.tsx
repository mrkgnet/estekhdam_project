...
export default function ShowDataSideBarUser({
  response,
}: ShowDataSideBarUserProps) {
  ...
  const dynamicMenuTree = useMemo(() => {
    ...
    menus.forEach((menu) => {
      menuMap[menu.id] = {
        ...
        url: menu.customUrl
          ? menu.customUrl
          : `/resources/main-resource?category=${menu.slug}`,
        icon: ListCollapse,
    ...
    if (level === 1 && tabId) {
      if (
        ...
        if (
          ...
          const clickedTab = dynamicMenuTree.find(
            (t) => t.id === tabId
          );

          setActiveSubTabId(
            clickedTab?.subItems?.[0]?.id || null
          );
        } else {
          setActiveSubTabId(null);
        }
  ...
  const activeTab = dynamicMenuTree.find(
    (t) => t.id === activeTabId
  );
      ...
      {isOpen && (
        <div
          ...
          className={`fixed inset-x-0 mt-1.5 bottom-0 z-40 bg-gray-900/40 backdrop-blur-sm
            ${isScrolled ? " top-0 md:top-12" : "top-31 md:top-29"}
          `}
          aria-hidden="true"
        />
      )}
      ...
      <aside
        ...
        className={`fixed right-0 z-70 w-full md:w-[280px] bg-white mt-1 flex flex-col shadow-xl border-l border-gray-100
          ${isScrolled
            ...
            : "top-31  md:top-30 h-[calc(100vh-8rem)]"
          }
              ...
              dynamicMenuTree.map((tab) => {
                ...
                return (
                  <div
                    key={tab.id}
                    onMouseEnter={() =>
                      handleInteraction(tab.id, 1)
                    }
                    onClick={() =>
                      handleInteraction(tab.id, 1, true)
                    }
