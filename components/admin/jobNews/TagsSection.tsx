...
export default function TagsSection({ jobs, setJobs, jobInput, setJobInput, cities, setCities, cityInput, setCityInput }: Props) {
    ...
    return (
        <div className="grid md:grid-cols-2 gap-6">
            {/* شغل ها */}
            <div className="bg-white p-6 sm:p-8 rounded shadow-sm hover:shadow-md transition-shadow duration-300 border border-slate-400 space-y-4">
                ...
                <div className="flex gap-2">
                    <input value={jobInput} onChange={(e) => setJobInput(e.target.value)} onKeyDown={(e) => handleKeyDown(e, addJob)} placeholder="مثلا: آموزگار ابتدایی (Enter)" className="flex-1 border border-slate-400 rounded-xl p-3 focus:ring-2 focus:ring-indigo-500 outline-none" />
                ...
                <div className="flex flex-wrap gap-2 pt-2 min-h-[40px] items-start">
                    ...
                    {jobs.map((job, i) => (
                        <span key={i} className="bg-slate-50 text-slate-700 border border-slate-400 px-3 py-1.5 rounded-lg flex items-center gap-2 group">
            ...
            <div className="bg-white p-6 sm:p-8 rounded shadow-sm hover:shadow-md transition-shadow duration-300 border border-slate-400 space-y-4">
                ...
                <div className="flex gap-2">
                    <input value={cityInput} onChange={(e) => setCityInput(e.target.value)} onKeyDown={(e) => handleKeyDown(e, addCity)} placeholder="مثلا: تهران (Enter)" className="flex-1 border border-slate-400 rounded-xl p-3 focus:ring-2 focus:ring-rose-500 outline-none" />
