...
function ShowDataResourcesSkeleton() {
...
export default async function page({ searchParams }: { searchParams: Promise<{ query?: string, category?: string, page?: string }> }) {
  ...
  const categoryQuery = params?.category || ""; // <--- دریافت کتگوری از URL
  const limit = 10;



  
  // اضافه کردن کتگوری به کلید یکتا برای رفرش شدن کش هنگام تغییر دسته‌بندی
  const suspenseKey = `${searchQuery}-${currentPage}-${categoryQuery}`;

  return (
    <div>
      <Suspense key={suspenseKey} fallback={<ShowDataResourcesSkeleton />}>
        <FetchDataUser 
          currentPage={currentPage} 
          searchQuery={searchQuery} 
          categoryQuery={categoryQuery} // <--- پاس دادن به کامپوننت فرزند
          limit={limit} 
        />
      </Suspense>
    </div>
