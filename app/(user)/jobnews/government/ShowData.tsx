...
/**
 * کامپوننت لوگو با بارگذاری بهینه و Shimmer خالص بدون تگ Style
 */
function BrandLogo({ src, alt }: { src?: string | null; alt: string }) {
  ...
  return (
    <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
      {/* لایه اسکلتون در زمان بارگذاری */}
      {!isLoaded && !hasError && (
        <div className="absolute inset-0 z-10 animate-pulse bg-slate-200 dark:bg-slate-700" />
      )}
...
function InfoChip({
  ...
  return (
    <div className="rounded-lg bg-slate-50/80 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors duration-200 px-3 py-2 border border-slate-300 dark:border-slate-700">
      <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
        <span className="inline-flex h-6 w-6 items-center justify-center rounded-md bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 shadow-sm">
          {icon}
        </span>
        <span className="text-xs">{label}</span>
      </div>
      <p className="mt-1 text-slate-900 dark:text-slate-100 font-medium text-sm truncate">
        {value}
      </p>
    </div>
  );
...
export default function ShowData({
  ...
  const news = useMemo(() => initialNews?.data || [], [initialNews?.data]);
  ...
  const activeRegions = useMemo(
    () => regionsQuery.split(",").filter(Boolean),
    [regionsQuery]
  );
  const activeStatuses = useMemo(
    () => statusesQuery.split(",").filter(Boolean),
    [statusesQuery]
  );
    ...
    return news.filter((x) => {
      const okRegion =
        activeRegions.length === 0
          ? true
          : x.cities?.some((c) => activeRegions.includes(c));
  ...
  if (!initialNews.success && news.length === 0) {
    return (
      <div className="p-8 mt-10 text-center bg-white dark:bg-slate-900 rounded-lg border border-slate-300 dark:border-slate-700 shadow-sm max-w-2xl mx-auto">
        <div className="mx-auto w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center justify-center mb-4 border border-slate-300 dark:border-slate-700">
          <CalendarRange className="h-8 w-8 text-slate-400 dark:text-slate-500" />
        </div>
        <p className="text-slate-600 dark:text-slate-400 font-medium">
          {initialNews.message || "اطلاعاتی یافت نشد"}
        </p>
        <button
          type="button"
  ...
  return (
    <div className="w-full max-w-7xl mx-auto sm:px-6 mt-4">
      <div className="grid grid-cols-12 gap-4 p-4 md:p-2">
        ...
        {/* بخش محتوا و نتایج */}
        <main className="col-span-12 lg:col-span-9 relative min-h-[400px]">
          {/* لودر فیلتر */}
          <AnimatePresence>
            {isPending && (
              <motion.div
              ...
              >
                <div className="flex flex-col items-center gap-3 p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800">
          ...
          {/* هدر وضعیت نتایج */}
          <div className="flex flex-col gap-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 p-4 mb-5 rounded-lg shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                ...
                <AnimatePresence>
                  {activeStatuses.map((s) => {
                    const label =
                      STATUS_OPTIONS.find((opt) => opt.key === s)?.label || s;
                    return (
                      <motion.span
                        key={`status-${s}`}
          ...
          <div className="space-y-5">
            {filtered.map((x) => (
              <div
                key={x.id}
                className="group rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-3 hover:shadow-xl hover:border-emerald-400 dark:hover:border-emerald-600 transition-all duration-300"
              >
                <Link
                ...
                >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 flex flex-wrap items-center gap-2 mt-1 text-sm">
                          ...
                          <span className="inline-flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded-md border border-slate-300 dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <InfoChip
                      ...
                      label="شرط سنی"
                      value={x.maxAge ? `حداکثر ${x.maxAge} سال` : "ندارد"}
                    />
                  </div>

                  {x.jobs && x.jobs.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                      {x.jobs.map((t, i) => (
                        <span
                          key={i}
                          className="rounded-md bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 px-3 py-1 font-medium text-slate-600 dark:text-slate-300 text-xs transition-colors"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  <div className="flex justify-end pt-1">
                    <span className="inline-flex items-center justify-center gap-2 rounded-lg px-6 py-2.5 bg-emerald-600 group-hover:bg-emerald-500 shadow-md text-white text-sm font-medium transition-all duration-300 w-full sm:w-auto">
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-dashed border-slate-300 dark:border-slate-700">
                <div className="mx-auto w-16 h-16 bg-white dark:bg-slate-800 rounded-lg flex items-center justify-center mb-4 shadow-sm border border-slate-300 dark:border-slate-700">
                  <MapPin className="h-8 w-8 text-slate-300 dark:text-slate-500" />
                </div>
                <p className="text-slate-500 dark:text-slate-400 font-medium">
                  آگهی استخدامی با این فیلترها یافت نشد.
                </p>
              </div>
            )}
          </div>
