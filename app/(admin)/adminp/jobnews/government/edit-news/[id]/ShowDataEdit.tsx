...
export default function EditShowJobNewsGov({
  getDataGov,
  getDataProduct,
}: {
  getDataGov: any;
  ...
  const [startAt, setStartAt] = useState<DateObject | null>(() => {
    return product.startAt
      ? new DateObject({ date: new Date(product.startAt), calendar: persian, locale: persian_fa })
      : null;
  });

  const [endAt, setEndAt] = useState<DateObject | null>(() => {
    return product.endAt
      ? new DateObject({ date: new Date(product.endAt), calendar: persian, locale: persian_fa })
      : null;
  });

  const [examAt, setExamAt] = useState<DateObject | null>(() => {
    return product.examAt
      ? new DateObject({ date: new Date(product.examAt), calendar: persian, locale: persian_fa })
      : null;
  });
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 relative text-xs sm:text-sm">
      ...
      <form action={formAction} className="space-y-6">
        ...
        <input type="hidden" name="examAt" value={toISO(examAt)} />
