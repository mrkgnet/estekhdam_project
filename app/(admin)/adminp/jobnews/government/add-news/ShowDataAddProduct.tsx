...
export default function CreateNews({ getDataProduct = [] }: Props) {
  ...
  const [examAt, setExamAt] = useState<DateObject | null>(null);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 relative text-xs md:text-sm">
      ...
      <form action={formAction} className="space-y-6">
        ...
        <input type="hidden" name="examAt" value={examAt ? examAt.toDate().toISOString() : ""} />
        <input type="hidden" name="isMainSlider" value={String(isMainSlider)} />

        {state?.success === false && (
          <div className="p-4 bg-red-50 text-red-700 rounded border border-red-100 flex items-center gap-2">
        ...
        <ScheduleSettingsSection
          ...
          setIsMainSlider={setIsMainSlider}
        />
        ...
        <div className="sticky bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-slate-200 p-4 sm:p-5 flex justify-end z-40 shadow-[0_-10px_30px_-10px_rgba(0,0,0,0.05)] -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 mt-12">
          <button
            type="submit"
            disabled={isPending}
            className="flex items-center justify-center gap-2 w-full sm:w-auto min-w-[200px] bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/30 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isPending ? (
              ...
              <span className="flex items-center gap-2">
                <Save className="w-5 h-5" /> ذخیره و انتشار آگهی
              </span>
            )}
          </button>
        </div>
